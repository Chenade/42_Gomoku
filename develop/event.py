    ...
    def handle_button(self):
        ...
        if event.startswith("start_pvc"):
            if event == "start_pvc_easy":
                self.game.ai = AIPlayer(depth=AI_DEPTH_EASY, col=self.game.setting.COL)
            elif event == "start_pvc_medium":
                self.game.ai = AIPlayer(depth=AI_DEPTH_MEDIUM, col=self.game.setting.COL)
            elif event == "start_pvc_hard":
                self.game.ai = AIPlayer(depth=AI_DEPTH_HARD, col=self.game.setting.COL)
